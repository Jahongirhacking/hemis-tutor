...
import { IProfile } from '../users/type';
...
export const chatApi = chatBaseApi.injectEndpoints({
  endpoints: build => ({
    ...
    getAllowedUniversities: build.query<string[], Pick<IChatReqBody, 'token'>>({
      query: params => ({
        url: `${chatBaseUrl}/allowed-universities`,
        params: { token: encodeStudentId(params?.token) },
      }),
    }),
    ...
    getScholarshipInfo: build.query<
      ...
      { studentId: IProfile['student_id_number'] }
    >({
      query: ({ studentId }) => ({
...
export const {
  ...
  useGetAllowedUniversitiesQuery,
