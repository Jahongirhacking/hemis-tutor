import { useGetStudentExtraInfoQuery, useGetStudentProfileQuery } from '@/services/api/statApi';
...
const useStudentPassport = ({ id }: { id: string }) => {
  const { data: studentDetails, isFetching: isStudentDetailsLoading } = useGetStudentDetailsQuery({ id: Number(id) }, { skip: !id });
  const studentId = useMemo(() => studentDetails?.result?.student?.student_id_number, [studentDetails]);
  const { data: studentData, isFetching: isPersonalLoading } = useGetStudentProfileQuery(
    { student_id_number: studentId },
    { skip: !studentId }
  );
  const student = useMemo(() => ({ ...(studentData?.data || {}), __details: { ...(studentDetails?.result || {}) } }), [studentData]);
  ...
  const { data: studentInfoData, isFetching: isStudentLoading } = useGetStudentExtraInfoQuery({ student_hash_number: student?.id }, { skip: !student?.id })
  ...
  const isLoading = isPersonalLoading || isStudentLoading || isStudentDetailsLoading

  const avatarUrl = useMemo(() => studentDetails?.result?.student?.image || (student?.fullName
    ? `https://ui-avatars.com/api/?name=${encodeURIComponent(student?.fullName)}&background=1f2937&color=fff&rounded=true`
    : PLACEHOLDER_AVATAR), [student, studentData])
  ...
  const registryBlocks = [
    ...
    },
    {
      ...
      items: hasSchoolCertificateData ? schoolCertificateSummary(studentInfo) : []
    },
    {
  ...
  const contactInfo = [
    {
      ...
      label: 'Manzil',
      value:
        [student?.region, student?.district, student?.address].filter(Boolean).join(', ') ||
        "Ma'lumot yo'q"
    },
    {
      icon: <Phone size={12} />,
...
function youthSummary(
  studentInfo: IStudentExtraInfoRes['data']
) {
...
function schoolCertificateSummary(
  studentInfo: IStudentExtraInfoRes['data']
) {
    ...
    ? [
      ...
      },
      {
        ...
        value: profEmis.is_have_diploma
          ? [profEmis.diploma_serial, profEmis.diploma_number].filter(Boolean).join(' ')
          : "Yo'q"
      },
      {
        label: "Ta'lim turi",
...
function creditModuleSummary(
  ...
  return creditModule
    ? [
      ...
      {
        label: 'Jami summa',
        value: creditModule.amount ? `${creditModule.amount.toLocaleString('uz-UZ')} so'm` : '-'
      },
      {
        label: 'Fanlar',
        value: creditModule.subjects?.length ? creditModule.subjects.join(', ') : '-'
      }
    ]
    : []
}
...
function orphanageSummary(
  studentInfo: IStudentExtraInfoRes['data']
) {
  const orphanage = studentInfo?.ORPHANAGE
  const count = orphanage?.Children?.length ?? 0
  const pinflList = orphanage?.Children?.map((child) => child.ChildPinfl).join(', ') ?? '-'
  const reasons = orphanage?.Children?.map((child) => child.LessReason).join(', ') ?? '-'
  const types = orphanage?.Children?.map((child) => child.GuardianshipType).join(', ') ?? '-'
...
function rentSummary(
  ...
  const totalAmount = rentPayments.reduce((sum, payment) => sum + (payment.amount || 0), 0)
  ...
  return [
    {
      ...
      value: totalAmount > 0 ? `${totalAmount.toLocaleString('uz-UZ')} so‘m` : '-'
    },
    {
...
function contractSummary(
  ...
  return [
    ...
    {
      label: 'Debit',
      value: contract.debit ? `${contract.debit.toLocaleString('uz-UZ')} so'm` : '-'
    }
    // {
    //   label: 'Credit',
