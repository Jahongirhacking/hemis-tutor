...
const ContractTypeCard = ({
    ...
    const courseData =
        Array.from((data?.result?.course_statistics || [])?.reduce<Map<number, IGrantType>>((acc, curr) => {
            curr?.grant_types?.forEach((t) => {
                if (acc.has(t?.grant_type_code)) {
                    acc.get(t?.grant_type_code).count += t?.count;
                } else {
                    acc.set(t?.grant_type_code, { ...t });
                }
            })
            return acc;
        }, new Map())?.values())?.map(item => ({
            name: item?.grant_type_name,
            value: item.count,
