...
const DistrictsCard = ({ isDark, PRIMARY, COLORS, ...props }: IStatisticsCardProps) => {
  const { data, isFetching } = useGetDashboardStatisticsQuery({
    expand: `${ExpandItem.DISTRICT_STATISTICS}`,
  });
  const districtData = useMemo(() => data?.result?.district_statistics?.map(stat => ({
    name: stat?.district_name,
    value: stat?.percent, // or stat.percent if you prefer
  })), [data]);

  const maxPercent = useMemo(() => data?.result?.district_statistics?.reduce((acc, curr) => Math.max(acc, curr?.percent), 0), [data?.result?.district_statistics]);
      ...
      <Flex vertical gap={12}>
        {
          isFetching ? (
            ...
            <ResponsiveContainer width="100%" height={districtData?.length <= 10 ? 240 : 180}>
              <PieChart>
                <Pie
                  data={districtData}
                  dataKey="value"
        ...
        <Flex vertical gap={12} className='overflow-y-auto max-h-[250px] pr-3' style={{ scrollbarColor: '#14b8a571 transparent' }}>
          {isFetching ? (
