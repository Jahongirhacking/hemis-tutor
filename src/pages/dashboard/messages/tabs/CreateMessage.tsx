...
const CreateMessagePage = () => {
    ...
    const studentRecipients = useMemo(() => recipientsData?.result?.recipients?.filter(r => r?.type === RecipientType.STUDENT), [recipientsData]);
    const employeeRecipients = useMemo(() => recipientsData?.result?.recipients?.filter(r => r?.type !== RecipientType.STUDENT), [recipientsData]);

    const handleChangeAlgorithm = useCallback((values: IRecipientType[]) => {
        setSelected(prev => {
            ...
            const currentIds = recipientsData?.result?.recipients?.map(o => o?.id) ?? [];
                    ...
                    <Select
                        ...
                        options={[
                            ...(recipientsData?.result?.recipients?.length ? [
                                { label: <Typography.Text style={{ color: '#3bb139' }} className="flex gap-2 items-center line-clamp-1"><SquareCheck size={14} /> {`${t('const.all')}`}</Typography.Text>, value: ALL_ITEMS },
                            ] : []),
                            ...(recipientsData?.result?.recipients?.length && selected?.length ? [
                                { label: <Typography.Text type="danger" className="flex gap-2 items-center line-clamp-1"><SquareX size={14} /> {`${t('const.clean')}`}</Typography.Text>, value: CLEAR_ITEMS },
                            ] : []),
                            ...(studentRecipients?.length ? [
                                {
                                    ...
                                    options: studentRecipients?.map(r => ({
                                        label: <span className="flex gap-2 items-center line-clamp-1"><GraduationCap size={14} /> {`${r?.name} - ${r?.label}`}</span>,
                                        value: r?.id,
                            ...
                            ...(employeeRecipients?.length ? [
                                {
                                    ...
                                    options: employeeRecipients?.map(r => ({
                                        label: <span className="flex gap-2 items-center line-clamp-1"><UserStar size={14} /> {`${r?.name} - ${r?.label}`}</span>,
                                        value: r?.id,
                                        title: `${r?.name} - ${r?.label}`
                                    }))
                                }
                        ...
                        loading={isFetching}
                        notFoundContent={isFetching ? <Spin size="small" /> : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={`${toFirstCapitalLetter(t('const.not_found'))}`} />}
                        mode="multiple"
                    />
                </Flex>
            </CustomFilter>
