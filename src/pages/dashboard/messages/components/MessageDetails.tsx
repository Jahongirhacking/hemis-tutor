...
const MessageDetails = ({ messageDetails }: { messageDetails: IMessageDetail }) => {
    ...
    return (
        <Flex vertical gap={12} className="w-full message-details-container">
            ...
            <Flex vertical gap={6}>
                ...
                <Typography.Text type="secondary" className="line-clamp-1">{`Kimga: ${messageDetails?.recipients?.map(r => r?.name)?.join(', ')}`}</Typography.Text>
                <Button type="text" icon={<Calendar1 size={15} />} className="flex p-0 gap-2 w-min">{moment(messageDetails?.created_at, `${CURRENT_DATE_FORMAT} HH:mm:ss`).format(`${CORRECT_DATE_FORMAT} HH:mm:ss`)}</Button>
            </Flex>
            <span className="message-content" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(messageDetails?.message) }} />
        </Flex>
    )
}
